...
export function StarterLogo({ size = 'md', showWordmark = true, className }: StarterLogoProps) {
  ...
  return (
    <div className={`starter-logo ${className ?? ''}`.trim()}>
      <svg
        width={iconSize}
        height={iconSize}
        viewBox="0 0 64 64"
        role="img"
        aria-label="Starter logo"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          <linearGradient id="starter-ring" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#2ea36c" />
            <stop offset="100%" stopColor="#1f6f4a" />
          </linearGradient>
          <linearGradient id="starter-flame" x1="0" y1="0" x2="1" y2="1">
        ...
        <path
          ...
          fill="url(#starter-flame)"
        />
      </svg>
      {showWordmark ? (
        <span className="starter-logo__wordmark">
          <span className="starter-logo__name">Starter</span>
          <span className="starter-logo__tag">Fullstack Typescript Kit</span>
        </span>
      ) : null}
    </div>
  );
